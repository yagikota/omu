...
int main(void) {
    FILE *fp;
    // step1
    fp = fopen("yagi_kota_task9_2.csv", "a");
    int N;
    ...
    switch (choice) {
        ...
        case 2:
            ...
            for (i = 0; i < N; i += 2)
            {
                r1 = (double)(rand() + 1) / (RAND_MAX + 1.0);
                r2 = (double)(rand() + 1) / (RAND_MAX + 1.0);
                u1 = sqrt(-2.0 * log(r1)) * cos(2.0 * M_PI * r2);
                u2 = sqrt(-2.0 * log(r1)) * sin(2.0 * M_PI * r2);
                x1 = myu + sigma * u1;
                x2 = myu + sigma * u2;
                x[i] = x1;
                x[i + 1] = x2;
